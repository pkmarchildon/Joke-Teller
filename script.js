...
function sayJoke(joke) {
   VoiceRSS.speech({
      key: '2c2cb3a8e2154d739173a75a6496e195',
      src: joke,
      hl: 'en-gb',
      r: 0,
      c: 'mp3',
...
async function fetchJoke() {
   ...
   let joke = '';
   try {
      const jokePromise = await fetch(apiUrl);
      const jokeObject = await jokePromise.json();
      // Verify if it is in 1 or 2 parts
      if (jokeObject.type === 'twopart') {
         // First part of the joke
         joke = `${jokeObject.setup} ... ${jokeObject.delivery}`;
         sayJoke(joke);
      } else {
         sayJoke(jokeObject.joke);
      }
   } catch (err) {
      console.log('Error happen while fetching joke', err);
   }
}
